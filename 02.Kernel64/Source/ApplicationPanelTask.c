...
static BOOL kCreateApplicationPanelWindow() {
  ...
  qwWindowID =
      kCreateWindow(0, 0, pstWindowManager->stScreenArea.iX2 + 1,
  ...
  kDrawRect(qwWindowID, 0, 0, pstWindowManager->stScreenArea.iX2,
            APPLICATIONPANEL_HEIGHT - 1, APPLICATIONPANEL_COLOR_OUTERLINE,
            FALSE);
  kDrawRect(qwWindowID, 1, 1, pstWindowManager->stScreenArea.iX2 - 1,
            APPLICATIONPANEL_HEIGHT - 2, APPLICATIONPANEL_COLOR_MIDDLELINE,
            FALSE);
  kDrawRect(qwWindowID, 2, 2, pstWindowManager->stScreenArea.iX2 - 2,
            APPLICATIONPANEL_HEIGHT - 3, APPLICATIONPANEL_COLOR_INNERLINE,
            FALSE);
  kDrawRect(qwWindowID, 3, 3, pstWindowManager->stScreenArea.iX2 - 3,
            APPLICATIONPANEL_HEIGHT - 4, APPLICATIONPANEL_COLOR_BACKGROUND,
            TRUE);
  ...
  while (1) {
    ...
    case EVENT_MOUSE_LBUTTONDOWN:
      ...
      if (kIsInRectangle(&(gs_stApplicationPanelData.stButtonArea),
                         pstMouseEvent->stPoint.iX,
                         pstMouseEvent->stPoint.iY) == FALSE)
        break;
      ...
      if (gs_stApplicationPanelData.bApplicationWindowVisible == FALSE) {
        ...
        kDrawButton(
            qwApplicationPanelID, &(gs_stApplicationPanelData.stButtonArea),
            APPLICATIONPANEL_COLOR_ACTIVE, "Application", RGB(255, 255, 255));
...
static void kDrawApplicationListItem(int iIndex, BOOL bMouseOver) {
  ...
  kSetRectangleData(
      0, iIndex * APPLICATIONPANEL_LISTITEMHEIGHT, iWindowWidth - 1,
      (iIndex + 1) * APPLICATIONPANEL_LISTITEMHEIGHT, &stItemArea);
...
static BOOL kProcessApplicationListWindowEvent(void) {
  ...
  while (1) {
    ...
    case EVENT_MOUSE_MOVE:
      ...
      if ((iMouseOverIndex ==
           gs_stApplicationPanelData.iPreviousMouseOverIndex) ||
          (iMouseOverIndex == -1))
        break;
    ...
    case EVENT_MOUSE_LBUTTONDOWN:
      ...
      kSetMouseEvent(qwApplicationPanelID, EVENT_MOUSE_LBUTTONDOWN,
                     ...
                     gs_stApplicationPanelData.stButtonArea.iY1 + 1, NULL,
                     &stEvent);
