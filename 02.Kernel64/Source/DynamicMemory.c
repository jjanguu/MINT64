...
void kInitializeDynamicMemory() {
  ...
  gs_stDynamicMemory.pstBitmapOfLevel =
      (BITMAP *)(DYNAMICMEMORY_START_ADDRESS +
                 (sizeof(BYTE) *
                  gs_stDynamicMemory.iBlockCountOfSmallestBlock));
...
BOOL kFreeMemory(void *pvAddress) {
  ...
  iBlockListIndex =
      (int)gs_stDynamicMemory.pbAllocatedBlockListIndex[iSizeArrayOffset];
...
DYNAMICMEMORY *kGetDynamicMemoryManager() { return &gs_stDynamicMemory; }
