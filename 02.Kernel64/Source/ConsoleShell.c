...
static void kTestThread(const char *pcParameterBuffer) {
  ...
  pstProcess =
      kCreateTask(TASK_FLAGS_LOW | TASK_FLAGS_PROCESS, (void *)0xEEEEEEEE,
...
static void kShowMatrix(const char *pcParameterBuffer) {
  ...
  pstProcess =
      kCreateTask(TASK_FLAGS_PROCESS | TASK_FLAGS_LOW, (void *)0xE00000,
                  0xE00000, (QWORD)kMatrixProcess, TASK_LOADBALANCINGID);
  if (pstProcess != NULL) {
...
static void kReadSector(const char *pcParameterBuffer) {
  ...
  if (kReadHDDSector(TRUE, TRUE, dwLBA, iSectorCount, pcBuffer) ==
      iSectorCount) {
...
static void kWriteSector(const char *pcParameterBuffer) {
  ...
  if (kWriteHDDSector(TRUE, TRUE, dwLBA, iSectorCount, pcBuffer) !=
      iSectorCount) {
    kPrintf("Write Fail\n");
    return;
  }
...
static void kDeleteFileInRootDirectory(const char *pcParameterBuffer) {
  ...
  if (remove(vcFileName) != 0) {
    kPrintf("File Not Found of File Opened\n");
    return;
  }
...
static void kTestFileIO(const char *pcParameterBuffer) {
  ...
  kPrintf("4. Sequntial Read And Verify Test(Cluster Size)...");
  ...
  for (i = 0; i < 100; i++) {
    ...
    kPrintf("    [%d] Offset [%d] Byte...", i, dwRandomOffset, dwByteCount);
  ...
  for (i = 0; i < (dwMaxFileSize / 1024); i++) {
    kPrintf("    [%d] Offset [%d] Byte [%d] Read And Verify...", i, i * 1024,
            1024);
...
static void kDownloadFile(const char *pcParameterBuffer) {
  ...
  while (dwReceivedSize < 4) {
    ...
    dwTempSize = kReceiveSerialData(((BYTE *)&dwDataLength) + dwReceivedSize,
                                    4 - dwReceivedSize);
...
static void kInstallPackage(const char *pcParameterBuffer) {
  ...
  if (kMemCmp(pstHeader->vcSignature, "MINT64OSPACKAGE ",
