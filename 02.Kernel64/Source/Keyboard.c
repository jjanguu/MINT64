...
BOOL kIsAlphabetScanCode(BYTE bScanCode){
    ...
    (gs_vstKeyMappingTable[bScanCode].bNormalCode <= 'z')) return TRUE;
    return FALSE;
}

BOOL kIsNumberOrSymbolScanCode(BYTE bScanCode){
    if((2 <= bScanCode) && (bScanCode <= 53) && (!kIsAlphabetScanCode(bScanCode))) return TRUE;
    return FALSE;
}
...
BOOL kIsUseCombinedCode(BYTE bScanCode){
    ...
    } else if(kIsNumberPadScanCode(bScanCode) && (!gs_stKeyboardManager.bExtendedCodeIn)){
...
BOOL kConvertScanCodeAndPutQueue(BYTE bScanCode){
    ...
    if(kConvertScanCodeToASCIICode(bScanCode, &(stData.bASCIICode), &(stData.bFlags))){
        bPreviousInterrrupt = kSetInterruptFlag(FALSE);

        bResult = kPutQueue(&gs_stKeyQueue, &stData);

        kSetInterruptFlag(bPreviousInterrrupt);
    }
    return bResult;
}
