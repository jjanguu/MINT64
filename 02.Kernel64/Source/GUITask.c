...
void kSystemMonitorTask() {
  ...
  qwWindowID = kCreateWindow(
      ...
      SYSTEMMONITOR_WINDOW_HEIGHT, WINDOW_FLAGS_DEFAULT & ~WINDOW_FLAGS_SHOW,
      "System Monitor");
  ...
  kDrawLine(qwWindowID, 5,
            WINDOW_TITLEBAR_HEIGHT + SYSTEMMONITOR_PROCESSOR_HEIGHT + 50,
            iWindowWidth - 5,
            WINDOW_TITLEBAR_HEIGHT + SYSTEMMONITOR_PROCESSOR_HEIGHT + 50,
            RGB(0, 0, 0));
  kDrawLine(qwWindowID, 5,
            WINDOW_TITLEBAR_HEIGHT + SYSTEMMONITOR_PROCESSOR_HEIGHT + 51,
            iWindowWidth - 5,
            WINDOW_TITLEBAR_HEIGHT + SYSTEMMONITOR_PROCESSOR_HEIGHT + 51,
            RGB(0, 0, 0));
  kDrawLine(qwWindowID, 5,
            WINDOW_TITLEBAR_HEIGHT + SYSTEMMONITOR_PROCESSOR_HEIGHT + 52,
            iWindowWidth - 5,
            WINDOW_TITLEBAR_HEIGHT + SYSTEMMONITOR_PROCESSOR_HEIGHT + 52,
            RGB(0, 0, 0));
  kDrawText(qwWindowID, 9,
            WINDOW_TITLEBAR_HEIGHT + SYSTEMMONITOR_PROCESSOR_HEIGHT + 43,
  ...
  while (1) {
    ...
    for (i = 0; i < iProcessorCount; i++) {
      ...
      if (bChanged == TRUE) {
        ...
        kDrawProcessorInformation(qwWindowID,
                                  i * SYSTEMMONITOR_PROCESSOR_WIDTH +
                                      (i + 1) * SYSTEMMONITOR_PROCESSOR_MARGIN,
    ...
    if (qwDynamicMemoryUsedSize != qwLastDynamicMemoryUsedSize) {
      ...
      kDrawMemoryInformation(qwWindowID,
                             WINDOW_TITLEBAR_HEIGHT +
                                 SYSTEMMONITOR_PROCESSOR_HEIGHT + 60,
                             iWindowWidth);
    }
  }
}
...
static void kDrawProcessorInformation(QWORD qwWindowID, int iX, int iY,
  ...
  iUsageBarHeight =
      (SYSTEMMONITOR_PROCESSOR_HEIGHT - 40) * qwProcessorLoad / 100;

  kDrawRect(qwWindowID, iX + 2,
            iY + (SYSTEMMONITOR_PROCESSOR_HEIGHT - iUsageBarHeight) - 2,
            iX + SYSTEMMONITOR_PROCESSOR_WIDTH - 2,
            iY + SYSTEMMONITOR_PROCESSOR_HEIGHT - 2, SYSTEMMONITOR_BAR_COLOR,
            TRUE);
...
static void kDrawMemoryInformation(QWORD qwWindowID, int iY, int iWindowWidth) {
  ...
  kDrawText(qwWindowID, SYSTEMMONITOR_PROCESSOR_MARGIN + 10, iY + 3,
            RGB(0, 0, 0), WINDOW_COLOR_BACKGROUND, vcBuffer, kStrLen(vcBuffer));

  kGetDynamicMemoryInformation(&qwDynamicMemoryStartAddress, &qwTemp, &qwTemp,
                               &qwDynamicMemoryUsedSize);

  kSPrintf(vcBuffer, "Used Size: %d KB        ",
           (qwDynamicMemoryUsedSize + qwDynamicMemoryStartAddress) / 1024);
  kDrawText(qwWindowID, SYSTEMMONITOR_PROCESSOR_MARGIN + 10, iY + 21,
            RGB(0, 0, 0), WINDOW_COLOR_BACKGROUND, vcBuffer, kStrLen(vcBuffer));
...
void kGUIConsoleShellTask(void) {
  ...
  iWindowHeight =
      CONSOLE_HEIGHT * FONT_ENGLISHHEIGHT + WINDOW_TITLEBAR_HEIGHT + 2;

  s_qwWindowID = kCreateWindow((stScreenArea.iX2 - iWindowWidth) / 2,
                               (stScreenArea.iY2 - iWindowHeight) / 2,
...
static void kProcessConsoleBuffer(QWORD qwWindowID) {
  ...
  for (j = 0; j < CONSOLE_HEIGHT; j++) {
    ...
    for (i = 0; i < CONSOLE_WIDTH; i++) {
      ...
      if ((pstScreenBuffer->bCharactor !=
           pstPreviousScreenBuffer->bCharactor) ||
          (bFullRedraw == TRUE)) {

        kDrawText(qwWindowID, i * FONT_ENGLISHWIDTH + 2,
                  j * FONT_ENGLISHHEIGHT + WINDOW_TITLEBAR_HEIGHT,
                  RGB(0, 255, 0), RGB(0, 0, 0), &(pstScreenBuffer->bCharactor),
                  1);
    ...
    if (bChanged == TRUE) {
      ...
      kSetRectangleData(2, j * FONT_ENGLISHHEIGHT + WINDOW_TITLEBAR_HEIGHT,
                        ...
                        (j + 1) * FONT_ENGLISHHEIGHT + WINDOW_TITLEBAR_HEIGHT -
                            1,
                        &stLineArea);
