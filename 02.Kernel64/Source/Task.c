...
static TCB *kAllocateTCB() {
  ...
  pstEmptyTCB->stLink.qwID =
      ((QWORD)gs_stTCBPoolManager.iAllocatedCount << 32) | i;
...
static void kFreeTCB(QWORD qwID) {
  ...
  index = GETTCBOFFSET(qwID);

  kMemSet(&(gs_stTCBPoolManager.pstStartAddress[index].stContext), 0,
          sizeof(CONTEXT));
  gs_stTCBPoolManager.pstStartAddress[index].stLink.qwID = index;
...
static void kSetUpTask(TCB *pstTCB, QWORD qwFlags, QWORD qwEntryPointAddress,
  ...
  kMemSet(pstTCB->stContext.vqRegister, 0,
...
void kInitializeScheduler() {
  ...
  pstTask->qwFlags =
      TASK_FLAGS_HIGHEST | TASK_FLAGS_PROCESS | TASK_FLAGS_SYSTEM;
  ...
  for (j = 0; j < 2; j++) {
    for (i = 0; i < TASK_MAXREADYLISTCOUNT; i++) {
      ...
      if (gs_stScheduler.viExecuteCount[i] < iTaskCount) {
        pstTarget =
            (TCB *)kRemoveListFromHeader(&(gs_stScheduler.vstReadyList[i]));
...
BOOL kChangePriority(QWORD qwTaskID, BYTE bPriority) {
  ...
  bPreviousFlag = kLockForSystemData(bPreviousFlag);
...
void kExitTask() { kEndTask(gs_stScheduler.pstRunningTask->stLink.qwID); }
...
int kGetTaskCount() {
  ...
  iTotalCount =
      kGetReadyTaskCount() + kGetListCount(&(gs_stScheduler.stWaitList)) + 1;
  bPrevioustFlag = kLockForSystemData();

  kUnlockForSystemData(bPrevioustFlag);
  return iTotalCount;
}
...
QWORD kGetProcessorLoad() { return gs_stScheduler.qwProcessorLoad; }
    ...
    if (kGetListCount(&(gs_stScheduler.stWaitList)) >= 0) {
      while (1) {
        ...
        if (pstTask->qwFlags & TASK_FLAGS_PROCESS) {
          ...
          for (i = 0; i < iCount; i++) {
            pstThreadLink =
                (TCB *)kRemoveListFromHeader(&(pstTask->stChildThreadList));
...
QWORD kGetLastFPUUsedTaskID() { return gs_stScheduler.qwLastFPUUsedTaskID; }
