...
static void kFreeTCB(QWORD qwID) {
  ...
  kMemSet(&(gs_stTCBPoolManager.pstStartAddress[i].stContext), 0,
          sizeof(CONTEXT));
...
void kInitializeScheduler(void) {
  ...
  if (bCurrentAPICID == 0)
    ...
    pstTask->qwFlags = TASK_FLAGS_LOWEST | TASK_FLAGS_PROCESS |
                       TASK_FLAGS_SYSTEM | TASK_FLAGS_IDLE;
...
static BYTE kFindSchedulerOfMinumumTaskCount(const TCB *pstTask) {
  ...
  iCurrentTaskCount = kGetListCount(
      &(gs_vstScheduler[pstTask->bAPICID].vstReadyList[bPriority]));
  ...
  for (i = 0; i < iProcessorCount; i++) {
    ...
    iTempTaskCount =
        kGetListCount(&(gs_vstScheduler[i].vstReadyList[bPriority]));
