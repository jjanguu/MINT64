...
void kPrintMPConfigurationTable(void) {
  ...
  char *vpcInterruptFlagsPO[4] = {"Conform", "Active High", "Reserved",
                                  "Active Low"};
  ...
  for (i = 0; i < pstMPTableHeader->wEntryCount; i++) {
    ...
    case MP_ENTRYTYPE_IOINTERRUPTASSIGNMENT:
      ...
      kPrintf("I/O Interrupt Flags : 0x%X ",
              pstIOAssignmentEntry->wInterruptFlags);

      kPrintf("(%s, %s)\n",
              vpcInterruptFlagsPO[pstIOAssignmentEntry->wInterruptFlags & 0x03],
              vpcInterruptFlagsEL[(pstIOAssignmentEntry->wInterruptFlags >> 2) &
                                  0x03]);
    ...
    case MP_ENTRYTYPE_LOCALINTERRUPTASSIGNMENT:
      ...
      kPrintf("Interrupt Type : 0x%X ",
              pstLocalAssignmentEntry->bInterruptType);

      kPrintf("(%s)\n",
              vpcInterruptType[pstLocalAssignmentEntry->bInterruptType]);
      kPrintf("I/O Interrupt Flags : 0x%X ",
              pstLocalAssignmentEntry->wInterruptFlags);

      kPrintf(
          "(%s, %s)\n",
          vpcInterruptFlagsPO[pstLocalAssignmentEntry->wInterruptFlags & 0x03],
          vpcInterruptFlagsEL[(pstLocalAssignmentEntry->wInterruptFlags >> 2) &
                              0x03]);
