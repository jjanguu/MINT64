...
void kInitializeSystemCall() {
  ...
  qwRDX =
      ((GDT_KERNELDATASEGMENT | SELECTOR_RPL_3) << 16) | GDT_KERNELCODESEGMENT;
...
QWORD kProcessSystemCall(QWORD qwServiceNumber, PARAMETERTABLE *pstParameter) {
  ...
  case SYSCALL_CREATETASK:
    pstTCB =
        kCreateTask(PARAM(0), (void *)PARAM(1), PARAM(2), PARAM(3), PARAM(4));
    if (pstTCB == NULL) {
  ...
  case SYSCALL_CREATEWINDOW:
    return kCreateWindow(PARAM(0), PARAM(1), PARAM(2), PARAM(3), PARAM(4),
                         (char *)PARAM(5));
  ...
  case SYSCALL_DRAWBUTTON:
    return kDrawButton(PARAM(0), (RECT *)PARAM(1), PARAM(2), (char *)PARAM(3),
                       PARAM(4));
  case SYSCALL_DRAWPIXEL:
    return kDrawPixel(PARAM(0), PARAM(1), PARAM(2), PARAM(3));
  case SYSCALL_DRAWLINE:
    return kDrawLine(PARAM(0), PARAM(1), PARAM(2), PARAM(3), PARAM(4),
                     PARAM(5));
  case SYSCALL_DRAWRECT:
    return kDrawRect(PARAM(0), PARAM(1), PARAM(2), PARAM(3), PARAM(4), PARAM(5),
                     PARAM(6));
  case SYSCALL_DRAWCIRCLE:
    return kDrawCircle(PARAM(0), PARAM(1), PARAM(2), PARAM(3), PARAM(4),
                       PARAM(5));
  ...
  case SYSCALL_GETCURSORPOSITION:
    ...
    return TRUE;
  case SYSCALL_BITBLT:
    return kBitBlt(PARAM(0), PARAM(1), PARAM(2), (COLOR *)PARAM(3), PARAM(4),
                   PARAM(5));
  ...
  case SYSCALL_READRTCDATE:
    kReadRTCDate((WORD *)PARAM(0), (BYTE *)PARAM(1), (BYTE *)PARAM(2),
                 (BYTE *)PARAM(3));
    return TRUE;
