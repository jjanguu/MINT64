...
static BOOL kReadClusterLinkTable(DWORD dwOffset, BYTE *pbBuffer) {
  return gs_pfReadHDDSector(
      TRUE, TRUE,
      dwOffset + gs_stFileSystemManager.dwClusterLinkAreaStartAddress, 1,
      pbBuffer);
}
...
static DWORD kFindFreeCluster() {
  ...
  for (i = 0; i < gs_stFileSystemManager.dwClusterLinkAreaSize; i++) {
    ...
    dwCurrentSectorOffset =
        (dwLastSectorOffset + i) % gs_stFileSystemManager.dwClusterLinkAreaSize;
...
static int kFindFreeDirectoryEntry() {
...
static BOOL kSetDirectoryEntryData(int iIndex, DIRECTORYENTRY *pstEntry) {
  ...
  if ((gs_stFileSystemManager.bMounted == FALSE) || (iIndex < 0) ||
      (iIndex >= FILESYSTEM_MAXDIRECTORYENTRYCOUNT))
    return FALSE;
...
DWORD kReadFile(void *pvBuffer, DWORD dwSize, DWORD dwCount, FILE *pstFile) {
  ...
  while (dwReadCount != dwTotalCount) {
    ...
    kMemCpy((char *)pvBuffer + dwReadCount, gs_vbTempBuffer + dwOffsetInCluster,
            dwCopySize);
...
DWORD kWriteFile(const void *pvBuffer, DWORD dwSize, DWORD dwCount,
                 FILE *pstFile) {
  DWORD dwWriteCount;
  DWORD dwTotalCount;
  DWORD dwOffsetInCluster;
  ...
  while (dwWriteCount != dwTotalCount) {
    ...
    } else if (((pstFileHandle->dwCurrentOffset % FILESYSTEM_CLUSTERSIZE) !=
                0) ||
    ...
    kMemCpy(gs_vbTempBuffer + dwOffsetInCluster,
            (char *)pvBuffer + dwWriteCount, dwCopySize);
...
struct kDirectoryEntryStruct *kReadDirectory(DIR *pstDirectory) {
  ...
  while (pstDirectoryHandle->iCurrentOffset <
