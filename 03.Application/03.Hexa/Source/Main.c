...
int Main(char *pcArgument) {
  ...
  DrawText(qwWindowID, 7, 200, RGB(255, 255, 255), RGB(0, 0, 0), pcStartMessage,
  ...
  while (1) {
    ...
    if (ReceiveEventFromWindowQueue(qwWindowID, &stEvent) == TRUE) {
      ...
      case EVENT_KEY_DOWN:
        ...
        case KEY_LEFT:
          if (IsMovePossible(g_stGameInfo.iBlockX - 1, g_stGameInfo.iBlockY) ==
              TRUE) {
    ...
    for (i = 0; i < BOARDWIDTH; i++) {
      ...
      if (g_stGameInfo.vvbBoard[j][i] != EMPTYBLOCK) {
        ...
        DrawRect(qwWindowID, i * BLOCKSIZE, iY + (j * BLOCKSIZE),
                 (i + 1) * BLOCKSIZE, iY + ((j + 1) * BLOCKSIZE), stColor,
                 TRUE);
    ...
    for (i = 0; i < BLOCKCOUNT; i++) {
      ...
      if (WINDOW_TITLEBAR_HEIGHT <
        ...
        DrawRect(qwWindowID, g_stGameInfo.iBlockX * BLOCKSIZE,
                 ...
                 iY + ((g_stGameInfo.iBlockY + i + 1) * BLOCKSIZE), stColor,
                 TRUE);
        ...
        DrawRect(qwWindowID, g_stGameInfo.iBlockX * BLOCKSIZE + 1,
                 ...
                 iY + ((g_stGameInfo.iBlockY + i + 1) * BLOCKSIZE) - 1, stColor,
                 FALSE);
      }
    }
  }
