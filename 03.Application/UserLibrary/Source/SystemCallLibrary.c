...
BOOL Schedule(void) { return (BOOL)ExecuteSystemCall(SYSCALL_SCHEDULE, NULL); }
...
BOOL UpdateScreenByWindowArea(QWORD qwWindowID, const RECT *pstArea) {
  ...
  return (BOOL)ExecuteSystemCall(SYSCALL_UPDATESCREENBYWINDOWAREA,
                                 &stParameter);
}
...
BOOL UpdateScreenByScreenArea(const RECT *pstArea) {
  ...
  return (BOOL)ExecuteSystemCall(SYSCALL_UPDATESCREENBYSCREENAREA,
                                 &stParameter);
}
...
BOOL ReceiveEventFromWindowQueue(QWORD qwWindowID, EVENT *pstEvent) {
  ...
  return (BOOL)ExecuteSystemCall(SYSCALL_RECEIVEEVENTFROMWINDOWQUEUE,
                                 &stParameter);
}
...
BOOL DrawWindowTitle(QWORD qwWindowID, const char *pcTitle,
                     BOOL bSelectedTitle) {
  PARAMETERTABLE stParameter;
...
BOOL DrawLine(QWORD qwWindowID, int iX1, int iY1, int iX2, int iY2,
              COLOR stColor) {
  PARAMETERTABLE stParameter;
...
void ClearSerialFIFO(void) { ExecuteSystemCall(SYSCALL_CLEARSERIALFIFO, NULL); }
